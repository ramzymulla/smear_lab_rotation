...
#SBATCH --job-name=rclone_for_kpms
#SBATCH --output=logs/rclone/rclone_kpms_%j.out
#SBATCH --error=logs/rclone/rclone_kpms_%j.err
#SBATCH -t0-24
#SBATCH -c 2
#SBATCH --mem=2G
...
for vid in $(rclone ls --include=${includeStr} rzadbx:thermister | awk '{print $2}'); do 
  ...
  [[ -s ${modelDir}/${modelName}/preds/${vidname/.avi/.slp} && ! -s ${vidRootDir}/${vidname} ]] && echo $vid 
done > ${tmpfile}
...
for vid in $(cat tmp.txt); do
  vidname=$(basename $vid)
  mv ${vidRootDir}/${vid} ${vidRootDir}/${vidname}
done

if [[ $includeStr =~ "raw" ]]; then
  sbatch $kpms_script 210 ${modelDir}/${modelName} $includeStr $vidRootDir
else
  sbatch $kpms_script 30 ${modelDir}/${modelName} $includeStr $vidRootDir
fi